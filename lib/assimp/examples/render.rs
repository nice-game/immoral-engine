...
use glium::{glutin, index::PrimitiveType, Surface};
...
fn main() {
...

	{
		...
		for mesh in scene.mesh_iter() {
			let verts: Vec<Vertex3> = mesh
				...
				.map(|(v, n)| Vertex3 { position: v.into(), normal: n.into() })
				.collect();
	...
	let draw = || {
		...
		for i in 0..vertex_buffers.len() {
			target.draw(&vertex_buffers[i], &index_buffers[i], &program, &uniforms, &params).unwrap();
		}
